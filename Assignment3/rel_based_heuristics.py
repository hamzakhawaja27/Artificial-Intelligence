...
class hff(_RelaxationHeuristic):
    ...
    def calc_goal_h(self):
        ...
        for key in self.facts.keys():
            if self.facts[key].name in self.goals:
                if self.facts[key].level==float('inf'):
                    print("Does not fulfill criteria")
                    break;
                elif max_level<self.facts[key].level:
                    max_level=self.facts[key].level
            else:
                pass
        ...
        for i in range(0,max_level+1):
            ...
            for key in self.facts.values():
                if key.name in self.goals and key.level==i:
                    goals_goals[i].add(key)

#        for gl in goals_goals.values():
#            for ggl in gl:
#                print(ggl.name)

        for i in range(max_level,0,-1):
            goal_set=goals_goals[i]
            for gg in goal_set:
                if gg.cheapest_achiever!=None:
                    relaxed_plan.add(gg.cheapest_achiever)
                    pre=gg.cheapest_achiever.preconditions
                    for p in pre:
                        for f in self.facts.values():
                            if f.name==p and f.level!=0:
#                                print(f.level)
                                goals_goals[f.level].add(f)
                                        

#        for jj in relaxed_plan:
#            print(jj.name)
...
class hadm(_RelaxationHeuristic):
    ...
    def __call__(self, node):
        ...
        for prop in self.facts.keys():
            if prop in state:
                cost_prop[prop]=0.0
            else:
                cost_prop[prop]=float('inf')

        while  cost_prop!=cost_prop1:
            for act in self.actions:
                cost_actions[act]=0.0
            
            cost_prop1=dict()
            cost_prop1=copy.deepcopy(cost_prop)
            for act in self.actions:
                for pre_prop in act.preconditions:
                    if cost_actions[act]<cost_prop[pre_prop]:
                        cost_actions[act]=cost_prop[pre_prop]


 
                for pos_prop in act.add_effects:
                    cost_prop[pos_prop]= min(cost_prop[pos_prop],cost_actions[act] + 1)



        heuristic=0
        for g in self.goals:
            if  heuristic < cost_prop[g]:
                heuristic=cost_prop[g]
